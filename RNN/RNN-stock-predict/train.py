...
def train():
    model = rnn(input_size=args.input_size, hidden_size=args.hidden_size, num_layers=args.layers , output_size=1, dropout=args.dropout, batch_first=args.batch_first)
    model.to(args.device) # 将模型（model）移动到指定设备（args.device）上的操作
    criterion = nn.MSELoss()  # 定义损失函数
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)  # Adam梯度下降  学习率
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    #plt.title('Training Loss Over Epochs-RNN without emotion')
    plt.title('Training Loss Over Epochs-RNN with emotion')
    plt.legend()
    #plt.savefig('image/RNN-withoutScore-training_loss_plot.png')  # 保存图像为PNG文件
    plt.savefig('image/RNN-withScore-training_loss_plot.png')  # 保存图像为PNG文件
    plt.show()
